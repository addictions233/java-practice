 ...
 * 1. 自定义JsonbTypeHandler类继承AbstractJsonTypeHandler类，重写parse和toJson方法。
 * 2. 在需要使用JSON类型的字段上增加@TableField(typeHandler = JsonbTypeHandler.class)注解进行类型的转换。
 * 3. 在实体类上增加autoResultMap = true属性开始自动映射。
 *
