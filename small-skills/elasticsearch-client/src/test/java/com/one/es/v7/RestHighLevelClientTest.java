...
/**
 * 然后接下来，从 ElasticSearch 7.15.0 版本开始，ElasticSearch 官方就不建议使用 High Level REST 客户端了，
 * 推荐使用 ElasticSearch-java 客户端了。
...
//        request.mapping(builder);
    ...
    public void insertData() {
        ...
        Gson gson = new Gson();
        //最好不要自定义id 会影响插入速度。
        request.id(productInfo.getId().toString());
        request.source(gson.toJson(productInfo)
                , XContentType.JSON);
        //endregion
    ...
    public void batchInsertData() {
        ...
        productInfo.setPrice(3999.00);
        productInfo.setDesc("xioami");
        for (int i = 0; i < 10; i++) {
            productInfo.setName("name" + i);
            request.add(new IndexRequest()
                    .id(Integer.toString(i))
                    .source(gson.toJson(productInfo)
                    , XContentType.JSON)
            );
        }
        //endregion
