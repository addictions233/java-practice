...
/**
 * 使用 TransportClient 作为 ES客户端, ES 7.x版本 开始已废弃
 * 从 ElasticSearch 7.0.0 版本开始，ElasticSearch 官方就不建议使用 TransportClient 客户端了
 * 推荐使用 High Level REST Client 客户端了。并且将要在 Elasticsearch 8.0 移除 TransportClient 客户端。
 */
@SpringBootTest
...
//        TransportClient client = new PreBuiltTransportClient(Settings.EMPTY)
    ...
    private void create(TransportClient client) {
        ...
        for (ProductInfo item : list) {
            System.out.println(item.getDate().format(DateTimeFormatter.ofPattern("yyyy-MM-dd HH:mm:ss")));
            IndexResponse response = client.prepareIndex("product", "_doc", item.getId().toString())
                    .setSource(XContentFactory.jsonBuilder()
                            ...
                            .field("date", item.getDate().format(DateTimeFormatter.ofPattern("yyyy-MM-dd")))
